...
\subsubsection{追问：这个高斯 Logprob 公式怎么来的}
...
\begin{summarybox}
\textbf{一句话：} 这个 logprob 就是在问：真实采样出来的下一步 $z_{t-1}$，在以模型均值 $\mu_\theta(z_t,t,x)$ 为中心、方差为 $\sigma_t^2 I$ 的高斯分布下，概率密度有多大。
\end{summarybox}

\begin{warnbox}
\textbf{和 SDE 记号的差别：} 如果一步更新写成
\[
x_{t-\Delta t}
=
\mu_\theta(x_t,t,c)
...
\]
那么高斯方差是 $\sigma_t^2\Delta t I$，logprob 里的分母和常数项也要变成：
\[
2\sigma_t^2\Delta t,
\qquad
\frac{d}{2}\log(2\pi\sigma_t^2\Delta t)
\]
本节 DDPO 公式里写的是 scheduler 已经给定一步方差 $\sigma_t^2 I$ 的形式。
\end{warnbox}
