...
\textbf{方向标签：} reasoning post-training；on-policy distillation；self-distillation；privileged context；dense token-level supervision；RLVR alternative。\\
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{opsd/opsd_tab1.png}
\caption{作者博客给出的训练范式对比图。它把 SFT、GRPO、普通蒸馏和 OPSD 放在同一张表里，最适合用来定位 OPSD：on-policy、dense token feedback、无外部 teacher、无需 outcome reward。}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth]{opsd/prompt_exampel_opsd.png}
\caption{作者博客中的 OPSD prompt 示例。关键点是 teacher 额外拿到 reference solution，但目标不是复述标准解，而是在理解标准解后对同一个 student prefix 给出 next-token distribution。}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\linewidth]{opsd/opsd_main.png}
\caption{OPSD 方法主图。左侧是 student 在只看题目时生成 on-policy rollout；右侧是同源 teacher 在额外看到标准解后，对 student rollout 的每个 prefix 产生软目标；最终用逐 token divergence 更新 student。}
\end{figure}
...
论文也讨论了一个替代目标：只在 student 实际采样出来的 token $\hat{y}_n$ 上计算 teacher 与 student 的 log-probability 差，并把它当成 token-level advantage：
...
\subsubsection{5.2 关键澄清：prefix 给 teacher 到底是什么意思}

这里最容易误解的一点是：\key{teacher 不是拿 student 的整篇回答去批改，也不是去模仿 student 的回答；student rollout 的作用，是暴露 student 自己真实会走到的一串中间状态。}

设 student 已经生成了一条回答：

\[
\hat y=(\hat y_1,\hat y_2,\ldots,\hat y_T).
\]
...
\hat y_{<2}=(\hat y_1),
\quad
\hat y_{<3}=(\hat y_1,\hat y_2),
\quad
\ldots,
\quad
\hat y_{<T}=(\hat y_1,\ldots,\hat y_{T-1}).
\]

对每一个位置 $n$，student 和 teacher 看的是\textbf{同一个 student prefix}，区别只在于 teacher 额外看到了标准解 $y^\star$：

\[
\underbrace{p_S(\cdot\mid x,\hat y_{<n})}_{\text{只看题目和 student prefix}}
\qquad\text{vs.}\qquad
\underbrace{p_T(\cdot\mid x,y^\star,\hat y_{<n})}_{\text{多看标准解，再看同一个 student prefix}}.
\]

所以 ``prefix 给 teacher'' 的真实含义是：

\begin{quote}
把 student 已经写到一半的状态 $\hat y_{<n}$ 也放进 teacher 的上下文里，让 teacher 在这个状态上输出下一 token 的概率分布。
\end{quote}

这不是让 teacher 从头生成标准答案，而是问 teacher 一个局部问题：

\begin{quote}
如果现在已经写到了这个 prefix，而且我知道标准解，那么下一步 token 的分布应该是什么样？
\end{quote}

继续用上面的买文具例子，如果 student 的完整回答是：
...
\begin{tabular}{lll}
\toprule
\textbf{位置} & \textbf{student prefix $\hat y_{<n}$} & \textbf{teacher 要做的事}\\
\midrule
早期 & ``5 支笔是 $5\times$'' & 看过标准解后，提高 token ``2'' 的概率，降低 ``3'' 的概率\\
中期 & ``5 支笔是 $5\times 3=15$ 元'' & 在错误已经发生的状态下，倾向输出 ``不过''、``这里应为'' 等纠错方向\\
后期 & ``所以总共是'' & 让分布更靠近正确答案 ``22''，而不是 ``27''\\
\bottomrule
\end{tabular}%
}
\end{center}

这个机制的关键价值是：\textbf{训练不只发生在标准答案轨迹上，而是发生在 student 自己会访问的轨迹上。} 这就是 OPSD 的 on-policy 部分。如果只在标准解 prefix 上训练，模型只学到 ``从正确前缀继续写''；而 OPSD 还让模型学到 ``当我自己写偏了，从这个偏掉的状态下一步该怎么拉回来''。

\begin{summarybox}
\textbf{我会这样记这个流程：} student 生成回答，不是为了让 teacher 模仿这份回答，而是为了产生一串真实的中间状态；teacher 拿着标准解进入这些状态，逐个给出 next-token distribution；最后训练 student 让它在这些状态下的分布靠近 teacher。
\end{summarybox}

\subsubsection{5.3 抽象成训练 iteration}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{opsd/main_result.png}
\caption{作者博客中的主结果图。它直观展示了 OPSD 在 Qwen3-1.7B、4B、8B 上相对 SFT 和 GRPO 的收益，尤其是小模型上 dense token-level supervision 的优势更明显。}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{opsd/opsd_tokenefficiency.png}
\caption{作者博客中的 token efficiency 对比图。OPSD 用更少 rollout token 获得 dense token-level soft targets，而 GRPO 需要多条长 rollout 才能通过最终 outcome reward 形成稀疏学习信号。}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\linewidth]{opsd/cliping.png}
\caption{作者博客中的 pointwise KL clipping 图。图中强调 style token 的 KL contribution 可能异常大；逐词表项裁剪可以避免训练被少数风格词主导，从而保护真正的数学 reasoning 信号。}
\end{figure}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{代码仓库：} \href{https://github.com/siyan-zhao/OPSD}{GitHub OPSD repository}，用于核对训练脚本、OPSDTrainer、评测设置和 README 中的复现实验说明。
